...
public class ExampleLevelMaker extends Application {
    ...
    private Level makeLevel () {
        ...
        /*
         * //Star:
         * Sprite star = new Sprite(new Point2D(2525.0, 50.0), Point2D.ZERO, new Dimension2D(50.0,
         * 50.0));
    ...
    private void doSwitch (Action gthc) {
        Sprite killSwitch =
                new Sprite(new Point2D(2100.0, 1700.0), Point2D.ZERO, new Dimension2D(30.0, 30.0));
    ...
    private void setUpImage (Sprite sprite, String path, int width, int height) {
        ...
        sprite.spriteImage().addImage(ImageToInt2DArray
                                              .convertImageTo2DIntArray(img, width, height));
    }
    ...
    private void applyActionAll (Sprite sprite1, Sprite enemy, Action action) {
        ...
        myCT.addActionToMap(sprite1.collisionTag(), enemy.collisionTag(), INT.COLLISION_DOWN,
                            action);
        myCT.addActionToMap(enemy.collisionTag(), sprite1.collisionTag(), INT.COLLISION_UP, null);
        myCT.addActionToMap(sprite1.collisionTag(), enemy.collisionTag(), INT.COLLISION_RIGHT,
                            action);
        myCT.addActionToMap(enemy.collisionTag(), sprite1.collisionTag(), INT.COLLISION_LEFT, null);
        myCT.addActionToMap(sprite1.collisionTag(), enemy.collisionTag(), INT.COLLISION_LEFT,
                            action);
    ...
    private void setCollisionUp (Sprite sprite, Sprite platform, Action bounceAction) {
        myCT.addActionToMap(sprite.collisionTag(), platform.collisionTag(), INT.COLLISION_UP,
                            bounceAction);
        myCT.addActionToMap(platform.collisionTag(), sprite.collisionTag(), INT.COLLISION_DOWN,
                            null);
    }
    ...
    private void addProjectile () {
        ...
        ProjectileMotionComponent projComp =
                new ProjectileMotionComponent(myProjectileTemplate,
    ...
    private Sprite makePlatform (double x,
                                 double y,
                                 double width,
                                 double height,
                                 List<Sprite> listToAdd,
                                 String tag) {
        Sprite platform =
                new Sprite(new Point2D(x, y), Point2D.ZERO, new Dimension2D(width, height));
